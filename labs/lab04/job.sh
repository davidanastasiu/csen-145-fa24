...
for f in 0.20 0.15 0.10 0.05; do
  LF="sparsematmult-1000-${f}-1.log"
  if [ ! -f "${LOGDIR}/${LF}" ]; then
    echo ${LF%4}
    ${DIR}/sparsematmult 1000 1000 1000 ${f} -s >> ${LOGDIR}/${LF}
  fi
  for t in 28 24 20 16 14 12 8 4 2; do
  ...
  LF="sparsematmult-2000-${f}-1.log"
  if [ ! -f "${LOGDIR}/${LF}" ]; then
    echo ${LF%4}
    ${DIR}/sparsematmult 2000 530 5000 ${f} -s >> ${LOGDIR}/${LF}
  fi
  for t in 28 24 20 16 14 12 8 4 2; do
  ...
  LF="sparsematmult-900-${f}-1.log"
  if [ ! -f "${LOGDIR}/${LF}" ]; then
    echo ${LF%4}
    ${DIR}/sparsematmult 900 3500 575 ${f} -s >> ${LOGDIR}/${LF}
  fi
  for t in 28 24 20 16 14 12 8 4 2; do
